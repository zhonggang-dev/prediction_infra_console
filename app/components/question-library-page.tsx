...
function TopicPathBadges({ paths }: { paths: string[] }) { return <div className="domain-badges">{paths.length ? paths.map((path) => <span key={path}>{formatTopicPath(path)}</span>) : <span>—</span>}</div>; }
