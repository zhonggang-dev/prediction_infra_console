  ...
  return <ConsoleShell>
    ...
    <section className="panel question-panel">
      <div className="question-filters">
        ...
        <label><span>领域</span><select className="select" value={draft.domain} onChange={(event) => setDraft({ ...draft, domain: event.target.value })}><option value="">全部领域</option>{(result?.domainOptions ?? []).map((domain) => <option key={domain} value={domain}>{domain}</option>)}</select></label>
...
function QuestionTable({ items, onSelect }: { items: ConsoleGeneratedMarket[]; onSelect: (item: ConsoleGeneratedMarket) => void }) {
  return <div className="table-scroll"><table className="question-table"><thead><tr><th>题目</th><th>状态</th><th>领域</th><th>题型</th><th>阶段</th><th>end_at</th><th>下次调度</th><th>尝试</th></tr></thead><tbody>{items.map((item) => <tr key={item.generatedMarketId} onClick={() => onSelect(item)}><td className="question-primary"><strong>{item.question}</strong><small>{item.generatedMarketId} · {item.conditionId}</small></td><td><Status value={item.status} /></td><td><DomainBadges domains={item.domainHints} /></td><td>{item.questionType === "numeric_bucket" ? "数值型" : "事件型"}</td><td className="mono muted">{item.phase}</td><td className="mono muted">{formatTime(item.endAt)}</td><td className="mono muted">{formatTime(item.nextRunAt)}</td><td className="mono">{item.attemptCount}</td></tr>)}</tbody></table></div>;
}

function QuestionDetail({ market, onClose }: { market: ConsoleGeneratedMarket; onClose: () => void }) {
  const ruleSections = formatResolutionRules(market.resolutionRules);
  return <div className="question-detail"><div className="section-head"><div><p className="eyebrow">Question Detail</p><h2 className="section-title">题目详情</h2></div><button className="button" onClick={onClose}>关闭</button></div><div className="question-detail-meta"><Status value={market.status} /><span>{market.phase}</span><span>{market.questionType === "numeric_bucket" ? "数值型" : "事件型"}</span><DomainBadges domains={market.domainHints} /></div><div className="question-detail-copy"><h3>{market.question}</h3><p>{market.description}</p>{market.resolvedOutcomeId && <div className="question-final-answer"><span>最终答案</span><strong>{market.resolvedOutcomeId} · {market.resolvedOutcomeName}</strong>{market.resolutionQuote && <q>{market.resolutionQuote}</q>}</div>}<h4>结算规则</h4><div className="resolution-rule-sections">{ruleSections.map((section) => <section key={section.key}>{section.title && <h5>{section.title}</h5>}<p>{section.content}</p></section>)}</div></div><div className="detail-grid"><DetailValue label="题目 ID" value={market.generatedMarketId} /><DetailValue label="Condition ID" value={market.conditionId} /><DetailValue label="end_at" value={formatTime(market.endAt)} /><DetailValue label="下次调度" value={formatTime(market.nextRunAt)} /><DetailValue label="最近原因" value={market.lastReasonCode} /><DetailValue label="尝试次数" value={String(market.attemptCount)} /><DetailValue label="生成请求" value={market.generationRequestId || "—"} /><DetailValue label="事件去重标识" value={market.eventInstanceId} /></div><p className="question-identity-help">事件去重标识用于判断两道题是否描述同一个现实事件，避免重复生成和重复入库；它不是用户可编辑的业务字段。</p><div className="question-outcomes"><h4>选项</h4>{market.outcomes.map((outcome, index) => { const option = outcomeObject(outcome); const selected = option.id === market.resolvedOutcomeId; return <span className={selected ? "selected" : ""} key={index}>{option.id ? `${option.id} · ` : ""}{option.name}{selected && <b>最终答案</b>}</span>; })}</div></div>;
}
...
function DomainBadges({ domains }: { domains: string[] }) { return <div className="domain-badges">{domains.length ? domains.map((domain) => <span key={domain}>{domain}</span>) : <span>—</span>}</div>; }
