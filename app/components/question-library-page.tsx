...
export function QuestionLibraryPage() {
  ...
  return <ConsoleShell>
    ...
    <section className="question-summary">
      <span>共 <strong>{result?.total ?? "—"}</strong> 道题</span><span>当前页 <strong>{result?.items.length ?? "—"}</strong> 道</span><span>查询来自 <strong>Prediction Infra</strong></span>
    </section>
...
function StatusDistribution({ counts }: { counts: Record<string, number> }) {
  ...
  return <section className="status-distribution panel"><div className="status-donut" style={{ background: `conic-gradient(${stops.join(", ") || "#dfe5dd 0deg 360deg"})` }}><div><strong>{total}</strong><span>总题数</span></div></div><div className="status-legend">{entries.map(([status, count]) => <div className="status-legend-item" key={status}><i style={{ background: statusColors[status] ?? "#8b9992" }} /><span>{statusLabels[status] ?? status}</span><strong>{count}</strong><small>{total ? `${Math.round((count / total) * 100)}%` : "0%"}</small></div>)}</div></section>;
}

function QuestionTable({ items, onSelect }: { items: ConsoleGeneratedMarket[]; onSelect: (item: ConsoleGeneratedMarket) => void }) {
  return <div className="table-scroll"><table className="question-table"><thead><tr><th>题目</th><th>状态</th><th>题型</th><th>阶段</th><th>end_at</th><th>下次调度</th><th>尝试</th></tr></thead><tbody>{items.map((item) => <tr key={item.generatedMarketId} onClick={() => onSelect(item)}><td className="question-primary"><strong>{item.question}</strong><small>{item.generatedMarketId} · {item.conditionId}</small></td><td><Status value={item.status} /></td><td>{item.questionType === "numeric_bucket" ? "数值型" : "事件型"}</td><td className="mono muted">{item.phase}</td><td className="mono muted">{formatTime(item.endAt)}</td><td className="mono muted">{formatTime(item.nextRunAt)}</td><td className="mono">{item.attemptCount}</td></tr>)}</tbody></table></div>;
}

function QuestionDetail({ market, onClose }: { market: ConsoleGeneratedMarket; onClose: () => void }) {
  return <div className="question-detail"><div className="section-head"><div><p className="eyebrow">Question Detail</p><h2 className="section-title">题目详情</h2></div><button className="button" onClick={onClose}>关闭</button></div><div className="question-detail-meta"><Status value={market.status} /><span>{market.phase}</span><span>{market.questionType === "numeric_bucket" ? "数值型" : "事件型"}</span><span>{market.primaryDomain}</span></div><div className="question-detail-copy"><h3>{market.question}</h3><p>{market.description}</p><h4>结算规则</h4><p>{market.resolutionRules}</p></div><div className="detail-grid"><DetailValue label="题目 ID" value={market.generatedMarketId} /><DetailValue label="Condition ID" value={market.conditionId} /><DetailValue label="end_at" value={formatTime(market.endAt)} /><DetailValue label="下次调度" value={formatTime(market.nextRunAt)} /><DetailValue label="最近原因" value={market.lastReasonCode} /><DetailValue label="尝试次数" value={String(market.attemptCount)} /><DetailValue label="生成请求" value={market.generationRequestId || "—"} /><DetailValue label="事件去重标识" value={market.eventInstanceId} /></div><p className="question-identity-help">事件去重标识用于判断两道题是否描述同一个现实事件，避免重复生成和重复入库；它不是用户可编辑的业务字段。</p><div className="question-outcomes"><h4>选项</h4>{market.outcomes.map((outcome, index) => <span key={index}>{typeof outcome === "object" && outcome !== null && "name" in outcome ? String((outcome as { name?: unknown }).name) : String(outcome)}</span>)}</div></div>;
}
