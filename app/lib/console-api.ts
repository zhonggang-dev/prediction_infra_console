...
function mapGeneratedMarket(item: RawRecord): ConsoleGeneratedMarket {
  return {
    ...
    outcomes: Array.isArray(item.outcomes) ? item.outcomes : [], domainHints: Array.isArray(item.domain_hints) ? item.domain_hints.map(String) : [], topicPaths: Array.isArray(item.topic_paths) ? item.topic_paths.map(String) : [], primaryDomain: string(item.primary_domain),
